// Test to verify Supabase connection and CRUD operations
// This test creates a minimal Supabase client directly
...
async function testSupabaseCRUD() {
  ...
  try {
    ...
    try {
      ...
      if (customersError) {
        console.log('❌ Connection failed:', customersError.message);
        return;
      }
    ...
    const { data: updatedCustomer, error: updateCustomerError } = await supabase
      .from('customers')
      .update({ shop_name: 'Updated Test Customer', sync_status: 'synced', updated_at: new Date().toISOString() })
      .eq('customer_id', customerId)
      .select()
      .single();
    ...
    const { data: updatedItem, error: updateItemError } = await supabase
      .from('items')
      .update({ item_display_name: 'Updated Test Item', sync_status: 'synced', updated_at: new Date().toISOString() })
      .eq('item_id', itemId)
      .select()
      .single();

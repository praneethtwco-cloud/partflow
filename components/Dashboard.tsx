                        ...
                        <div key={`${customer.customerId || 'empty'}-${index}`} className={`rounded-xl md:rounded-2xl p-3 md:p-4 shadow-sm border ${index === 0 ? `bg-white border-l-4 ${themeClasses.border}` : 'bg-white border border-slate-100'} ${index > 0 ? 'opacity-90' : ''} ${isCompleted ? 'border-emerald-200 bg-emerald-50/30' : ''}`}>
                            <div className="flex items-start justify-between gap-2 md:gap-3">
                                ...
                                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                                    {customer.customerId && (
                                        <button onClick={() => onOpenProfile(customer.customerId)} aria-label="View customer profile" className="text-slate-400 hover:text-slate-700 p-1 rounded focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-indigo-500">
                ...
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-end md:items-center justify-center pb-24 md:pb-0">
                    <div className="bg-white rounded-t-3xl md:rounded-3xl w-full md:max-w-md p-6 animate-in slide-in-from-bottom-20 duration-300">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => setShowRoutePlanner(false)} aria-label="Close add route stop modal" className="p-2 text-slate-400 hover:text-slate-600 rounded focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-indigo-500">
                ...
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-end md:items-center justify-center pb-24 md:pb-0">
                    <div className="bg-white rounded-t-3xl md:rounded-3xl w-full md:max-w-md p-6 animate-in slide-in-from-bottom-20 duration-300">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => setShowCheckInModal(false)} aria-label="Close check in modal" className="p-2 text-slate-400 hover:text-slate-600 rounded focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-indigo-500">
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
                            </button>
                        </div>
